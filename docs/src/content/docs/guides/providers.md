...
- When you're *inside* a repo, its own worktrees are found via git first —
  providers matter when resolving from anywhere else.
- Conductor leaves a renamed workspace behind as a **symlink** to the new
  name; twig follows those and collapses duplicates to the canonical path.

## Codex and cloud sessions

Codex keeps no local worktree directory — its cloud sessions (and Claude
Code web sessions) exist as **branches on GitHub**. Those are covered by
[remote branch pickup](/twig/guides/remote-pickup/), not a provider.
