...
- The first time twig sees a given `twig.toml`, it **refuses to use it**,
  prints the full contents, and asks for approval — interactively (y/N) when
  you're at a terminal, otherwise with instructions to run `twig trust`.
- Approval is recorded as `sha256(absolute path + file content)`. **Any
  edit to the file — or moving it — invalidates the approval** and the
  gate trips again.
- The trust gate covers the *whole* manifest: an untrusted `twig.toml`
  contributes neither scripts **nor** its `[open]` section, because
  repo-defined openers are arbitrary commands too.
